...
func TestBoardControllerPost(t *testing.T) {
	...
	t.Run("should create board", func(t *testing.T) {
		...
		boardController := NewBoardController(*slog.Default(), board.NewBoardCreator(*slog.Default(), &boardRepositoryMock, &userRepositoryMock, &teamRepositoryMock))
		uuid, _ := uuid.NewV7()
	...
	t.Run("should return bad request", func(t *testing.T) {
		writter := httptest.NewRecorder()
		boardController := NewBoardController(*slog.Default(), board.NewBoardCreator(*slog.Default(), nil, nil, nil))
